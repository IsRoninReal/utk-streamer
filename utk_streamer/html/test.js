...
window.addEventListener("message", (event) => {
    if (event.data.type == "open") {
        ...
        console.log("stopstream", event.data);
...
document.getElementById("stop-stream").addEventListener("click", () => {
    if (streaming) {
        ...
        $.post("https://utk_streamer/tryJoinStream", JSON.stringify({streamId: streamId}), (response) => {
            if (response) {
                StartWatching(streamId, response);
            }
        })
        console.log("stopstream", streamId);
        streamId = 0;
        for (i in peers) {
            peers[i].RTC.close();
        }
